...
with open(input_file,newline="", encoding="utf-8") as budget:
     ...
     # Store the contents of budget_data.csv in variable csvreader
    csvreader = csv.reader(budget,delimiter=",") 

    # Skip the header labels to iterate  the values
    header = next(csvreader)  

    # Iterate through the rows in the stored file contents
    for row in csvreader: 

        # Append the total months and total profit to the corresponding lists
...
# Correlate max and min to the appropriate month using month list and index from max and min
